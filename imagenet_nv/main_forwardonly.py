    ...
    for epoch in range(args.start_epoch, args.epochs + 5):
        ...
        with warnings.catch_warnings():
            ...
            if args.forward_only:
                validate(train_loader, model, criterion, epoch, start_time)
            else:
...
class data_prefetcher():
    ...
    def preload(self):
        if self.cur_it < self.nb_total_it:
            self.cur_it += 1
            with torch.cuda.stream(self.stream):
                self.next_input = torch.cuda.FloatTensor(args.batch_size, 3, args.resol, args.resol).normal_()
                self.next_target = torch.cuda.LongTensor(args.batch_size)
        else:
            self.cur_it = 0
